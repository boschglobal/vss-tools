...
class Vspec2X:
    ...
    def main(self, arguments):
        ...
        parser.add_argument(
            ...
            "--strict",
            action="store_true",
            help="Use stricter checking: Terminate when anything not covered or not recommended "
        ...
        if (
            self.vspec2vss_config.expand_model
            and self.vspec2vss_config.no_expand_option_supported
        ):
            parser.add_argument(
                "--no-expand", action="store_true", help="Do not expand tree."
            )
        parser.add_argument(
            "-o",
            "--overlays",
        ...
        if self.vspec2vss_config.output_file_required:
            parser.add_argument(
                "output_file",
                metavar="<output_file>",
                help="The file to read and write output to.",
            )
        ...
        self.generator.add_arguments(
            parser.add_argument_group("Exporter specific arguments", "")
        )
        ...
        if self.vspec2vss_config.extended_attributes_supported and (
            len(args.extended_attributes) > 0
        ):
        ...
        self.vspec2vss_config.generate_uuid = (
            self.vspec2vss_config.uuid_supported and args.uuid
        )
        ...
        if self.vspec2vss_config.generate_uuid:
            ...
            logging.info(
                "If you need static identifiers consider using the vspec2id tool"
            )
    ...
    def processDataTypeTree(
        ...
        """
        if (
            self.vspec2vss_config.separate_output_type_file_supported
            and args.types_output_file is None
        ):
            logging.info(
                "Sensors and custom data types will be consolidated into one file."
            )

        first_tree = True
        for type_file in args.vspec_types_file:
            logging.info(
                f"Loading and processing struct/data type tree from {type_file}"
            )
            new_tree = vspec.load_tree(
                type_file,
                include_dirs,
